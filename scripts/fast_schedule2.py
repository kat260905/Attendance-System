...
def main():
    ...
    with engine.begin() as conn:
        ...
        # timetable
        tt = conn.execute(sa.text(f"SELECT id FROM timetables WHERE faculty_id={f_id} AND subject_id={s_id} AND day_of_week='Saturday'")).mappings().first()
        if not tt:
            conn.execute(sa.text(f"INSERT INTO timetables (subject_id, faculty_id, day_of_week, start_time, end_time) VALUES ({s_id}, {f_id}, 'Saturday', '10:00:00', '11:00:00')"))
            print("Created Saturday Timetable")
            
        # class_sessions map to class_id
